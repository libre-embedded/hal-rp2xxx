...
#pragma once
#ifndef RP2XXX_STRUCTS_I2C_H
#define RP2XXX_STRUCTS_I2C_H

#include "../enums/I2C_IC_CON_SPEED.h"
 ...
 * values, set at hardware design time): IC_ULTRA_FAST_MODE ................
 * 0x0 IC_UFM_TBUF_CNT_DEFAULT ........... 0x8 IC_UFM_SCL_LOW_COUNT
 * .............. 0x0008 IC_UFM_SCL_HIGH_COUNT ............. 0x0006 IC_TX_TL
 * .......................... 0x0 IC_TX_CMD_BLOCK ................... 0x1
 *         IC_HAS_DMA ........................ 0x1
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 2;     /*!< i2c's identifier. */
...
be written to only when IC_ENABLE[0] is set to 0. Note: If the software or
...
the CPU reads from when retrieving bytes from RX FIFO. The size of the register
changes as follows: Write: - 11 bits when IC_EMPTYFIFO_HOLD_MASTER_EN=1 - 9
bits when IC_EMPTYFIFO_HOLD_MASTER_EN=0 Read: - 12 bits when
IC_FIRST_DATA_BYTE_STATUS = 1 - 8 bits when IC_FIRST_DATA_BYTE_STATUS = 0 Note:
In order for the DW_apb_i2c to continue acknowledging reads, a read command
should be written for every byte that is to be received; otherwise the
...
Register Each bit in this register has a corresponding mask bit in the
...
Status Register Unlike the IC_INTR_STAT register, these bits are not masked so
...
this register request an interrupt. When the I2C is disabled by writing 0 in
bit 0 of the IC_ENABLE register: - Bits 1 and 2 are set to 1 - Bits 3 and 10
...
The bits [15:0] of this register are used to control the hold time of SDA
during transmit in both slave and master mode (after SCL goes from HIGH to
LOW). The bits [23:16] of this register are used to extend the SDA transition
(if any) whenever SCL is HIGH in the receiver in either master or slave mode.
...
implemented. The programmed SDA hold time during transmit (IC_SDA_TX_HOLD)
...
Register The register is used to generate a NACK for the data part of a
...
address has no effect. A write can occur on this register if both of the
...
This register controls the amount of time delay (in terms of number of ic_clk
       ...
       greater than 2.        Writes to this register succeed only when IC_ENABLE[0] = 0.
...
Register The register controls whether DW_apb_i2c responds with a ACK or NACK
when it receives an I2C General Call address. This register is applicable only
when the DW_apb_i2c is in slave mode. */
    const uint32_t IC_ENABLE_STATUS = {}; /*!< (read-only) I2C Enable Status
Register The register is used to report the DW_apb_i2c hardware status when the
IC_ENABLE[0] register is set from 1 to 0; that is, when DW_apb_i2c is disabled.
If IC_ENABLE[0] has been set to 1, bits 2:1 are forced to 0, and bit 0 is
forced to 1. If IC_ENABLE[0] has been set to 0, bits 2:1 is only be valid as
soon as bit 0 is read as '0'. Note: When IC_ENABLE[0] has been set to 0, a
delay occurs for bit 0 to be read as 0 because disabling the DW_apb_i2c depends
on I2C bus activities. */
    uint32_t IC_FS_SPKLEN; /*!< (read-write) I2C SS, FS or FM+ spike
suppression limit This register is used to store the duration, measured in
...
Register 1 Note This register is not implemented and therefore reads as 0. If
it was implemented it would be a constant read-only register that contains
...
below are the settings for those parameters */
     ...
     * is used to capture correct value of spike filter as per the speed mode.
     *                 This register should be programmed only with a value in
     * the range of 1 to IC_MAX_SPEED_MODE; otherwise, hardware updates this
     * register with the value of IC_MAX_SPEED_MODE. 1: standard mode (100
     * kbit/s) 2: fast mode (<=400 kbit/s) or fast mode plus (<=1000Kbit/s) 3:
     * high speed mode (3.4 Mbit/s) Note: This field is not applicable when
     * IC_ULTRA_FAST_MODE=1
     */
    inline I2C_IC_CON_SPEED get_IC_CON_SPEED() volatile
    {
        return I2C_IC_CON_SPEED((IC_CON >> 1u) & 0b11u);
    }
     ...
     * IC_ULTRA_FAST_MODE=1
     */
    inline void set_IC_CON_SPEED(I2C_IC_CON_SPEED value) volatile
    {
     ...
     * will result in setting bit 6 (TX_ABRT) of the IC_RAW_INTR_STAT register.
     *                 Reset value: ENABLED
     */
     ...
     * disabled. If this bit is set (slave is disabled), DW_apb_i2c functions
     * only as a master and does not perform any action that requires a slave.
     *                 NOTE: Software should ensure that if this bit is written
     ...
     * addressed or not. Reset value: 0x0 NOTE: During a general call address,
     * this slave does not issue the STOP_DET interrupt if
     ...
     * in the IC_RAW_INTR_STAT register. Reset value: 0x0.
     */
     ...
     * IC_RX_FULL_HLD_BUS_EN parameter. Reset value: 0x0.
     */
     ...
     * read only.
     */
    inline void get_IC_CON(bool &MASTER_MODE, I2C_IC_CON_SPEED &SPEED,
                           ...
                           bool &STOP_DET_IF_MASTER_ACTIVE) volatile
    {
        ...
        MASTER_MODE = curr & 1u;
        SPEED = I2C_IC_CON_SPEED((curr >> 1u) & 0b11u);
     ...
     * read only.
     */
    inline void set_IC_CON(bool MASTER_MODE, I2C_IC_CON_SPEED SPEED,
                           bool IC_10BITADDR_SLAVE, bool IC_10BITADDR_MASTER,
                           bool IC_RESTART_EN, bool IC_SLAVE_DISABLE,
                           bool STOP_DET_IFADDRESSED, bool TX_EMPTY_CTRL,
     ...
     * CPU needs to write only once into these bits. If the IC_TAR and IC_SAR
     * are the same, loopback exists but the FIFOs are shared between master
     * and slave, so full loopback is not feasible. Only one direction loopback
     * mode is supported (simplex), not duplex. A master cannot transmit to
     * itself; it can transmit to only a slave.
     */
    inline uint16_t get_IC_TAR() volatile
    {
        return IC_TAR & 0b1111111111u;
    }
     ...
     * itself; it can transmit to only a slave.
     */
    inline void set_IC_TAR(uint16_t value) volatile
    {
        uint32_t curr = IC_TAR;

        curr &= ~(0b1111111111u);
        curr |= (value & 0b1111111111u);
     ...
     * For 7-bit addressing, only IC_SAR[6:0] is used. This register can be
     ...
     * effect. Note: The default values cannot be any of the reserved address
     * locations: that is, 0x00 to 0x07, or 0x78 to 0x7f. The correct operation
     * of the device is not guaranteed if you program the IC_SAR or IC_TAR to a
     * reserved value. Refer to <<table_I2C_firstbyte_bit_defs>> for a complete
     * list of these reserved values.
     */
    inline uint16_t get_IC_SAR() volatile
    {
        return IC_SAR & 0b1111111111u;
    }
     ...
     * list of these reserved values.
     */
    inline void set_IC_SAR(uint16_t value) volatile
    {
     ...
     * DW_apb_i2c interface. Reset value: 0x0
     */
    inline uint8_t get_IC_DATA_CMD_DAT() volatile
    {
        return IC_DATA_CMD & 0xffu;
    }
     ...
     * receive transfer in Master receiver or Slave receiver mode. Reset value
     * : 0x0 NOTE:  In case of APB_DATA_WIDTH=8,
     ...
     * from when retrieving bytes from RX FIFO. The size of the register
     * changes as follows: Write: - 11 bits when IC_EMPTYFIFO_HOLD_MASTER_EN=1
     ...
     * 'IC_CLK Frequency Configuration'. This register can be written only when
     * the I2C interface is disabled which corresponds to the IC_ENABLE[0]
     ...
     * first. Then the upper byte is programmed. NOTE: This register must not
     * be programmed to a value higher than 65525, because DW_apb_i2c uses a
     * 16-bit counter to flag an I2C bus idle condition when this counter
     * reaches a value of IC_SS_SCL_HCNT + 10.
     */
    inline uint16_t get_IC_SS_SCL_HCNT() volatile
    {
        return IC_SS_SCL_HCNT & 0xffffu;
    }
     ...
     * reaches a value of IC_SS_SCL_HCNT + 10.
     */
    inline void set_IC_SS_SCL_HCNT(uint16_t value) volatile
    {
     ...
     * Frequency Configuration' This register can be written only when the I2C
     * interface is disabled which corresponds to the IC_ENABLE[0] register
     * being set to 0. Writes at other times have no effect. The minimum valid
     * value is 8; hardware prevents values less than this being written, and
     * if attempted, results in 8 being set. For designs with APB_DATA_WIDTH =
     * 8, the order of programming is important to ensure the correct operation
     * of DW_apb_i2c. The lower byte must be programmed first, and then the
     * upper byte is programmed.
     */
    inline uint16_t get_IC_SS_SCL_LCNT() volatile
    {
        return IC_SS_SCL_LCNT & 0xffffu;
    }
     ...
     * upper byte is programmed.
     */
    inline void set_IC_SS_SCL_LCNT(uint16_t value) volatile
    {
     ...
     * For more information, refer to 'IC_CLK Frequency Configuration'. This
     ...
     */
    inline uint16_t get_IC_FS_SCL_HCNT() volatile
    {
        return IC_FS_SCL_HCNT & 0xffffu;
    }
     ...
     * first. Then the upper byte is programmed.
     */
    inline void set_IC_FS_SCL_HCNT(uint16_t value) volatile
    {
     ...
     * 'IC_CLK Frequency Configuration'. This register goes away and becomes
     * read-only returning 0s if IC_MAX_SPEED_MODE = standard. This register
     ...
     * times have no effect. The minimum valid value is 8; hardware prevents
     * values less than this being written, and if attempted results in 8 being
     * set. For designs with APB_DATA_WIDTH = 8 the order of programming is
     * important to ensure the correct operation of the DW_apb_i2c. The lower
     * byte must be programmed first. Then the upper byte is programmed. If the
     * value is less than 8 then the count value gets changed to 8.
     */
    inline uint16_t get_IC_FS_SCL_LCNT() volatile
    {
        return IC_FS_SCL_LCNT & 0xffffu;
    }
     ...
     * value is less than 8 then the count value gets changed to 8.
     */
    inline void set_IC_FS_SCL_LCNT(uint16_t value) volatile
    {
     ...
     * interrupt is cleared. Reset value: 0x0
     */
    inline bool get_IC_RAW_INTR_STAT_RX_UNDER() volatile
    {
        return IC_RAW_INTR_STAT & 1u;
    }
     ...
     * and when ic_en goes to 0, this interrupt is cleared. Note:  If bit 9 of
     * the IC_CON register (RX_FIFO_FULL_HLD_CTRL) is programmed to HIGH, then
     * the RX_OVER interrupt never occurs, because the Rx FIFO never overflows.
     *                 Reset value: 0x0
     */
     ...
     * is programmed with a 0, regardless of the activity that continues. Reset
     * value: 0x0
     */
     ...
     * longer any activity, then with ic_en=0, this bit is set to 0. Reset
     * value: 0x0.
     */
     ...
     * reads the IC_CLR_RD_REQ register. Reset value: 0x0
     */
     ...
     * takes places. Note:  The DW_apb_i2c flushes/resets/empties the TX_FIFO
     * and RX_FIFO whenever there is a transmit abort caused by any of the
     * events tracked by the IC_TX_ABRT_SOURCE register. The FIFOs remains in
     * this flushed state until the register IC_CLR_TX_ABRT is read. Once this
     * read is performed, the Tx FIFO is then ready to accept more data bytes
     * from the APB interface. Reset value: 0x0
     */
     ...
     * slave is addressed. Note: During a general call address, this slave does
     * not issue a STOP_DET interrupt if STOP_DET_IF_ADDRESSED=1'b1, even if
     * the slave responds to the general call address by generating ACK. The
     * STOP_DET interrupt is generated only when the transmitted address
     * matches the slave address (SAR). - If IC_CON[7]=1'b0
     ...
     * (STOP_DET_IF_MASTER_ACTIVE),the STOP_DET interrupt will be issued only
     * if Master is active. - If IC_CON[10]=1'b0  (STOP_DET_IFADDRESSED),the
     * STOP_DET interrupt will be issued irrespective of whether master is
     * active or not. Reset value: 0x0
     */
     ...
     * the received data in the Rx buffer. Reset value: 0x0
     */
     ...
     * addressed. Enabled only when IC_SLV_RESTART_DET_EN=1. Note: However, in
     * high-speed mode or during a START BYTE transfer, the RESTART comes
     ...
     * DW_apb_i2c does not generate the RESTART_DET interrupt. Reset value: 0x0
     */
     ...
     */
    inline uint8_t get_IC_RX_TL() volatile
    {
        return IC_RX_TL & 0xffu;
    }
     ...
     * entry, and a value of 255 sets the threshold for 256 entries.
     */
    inline void set_IC_RX_TL(uint8_t value) volatile
    {
     ...
     */
    inline uint8_t get_IC_TX_TL() volatile
    {
        return IC_TX_TL & 0xffu;
    }
     ...
     * threshold for 255 entries.
     */
    inline void set_IC_TX_TL(uint8_t value) volatile
    {
     ...
     * IC_TX_ABRT_SOURCE. Reset value: 0x0
     */
    inline bool get_IC_CLR_INTR() volatile
    {
        return IC_CLR_INTR & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_RX_UNDER() volatile
    {
        return IC_CLR_RX_UNDER & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_RX_OVER() volatile
    {
        return IC_CLR_RX_OVER & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_TX_OVER() volatile
    {
        return IC_CLR_TX_OVER & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_RD_REQ() volatile
    {
        return IC_CLR_RD_REQ & 1u;
    }
     ...
     * exception to clearing IC_TX_ABRT_SOURCE. Reset value: 0x0
     */
    inline bool get_IC_CLR_TX_ABRT() volatile
    {
        return IC_CLR_TX_ABRT & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_RX_DONE() volatile
    {
        return IC_CLR_RX_DONE & 1u;
    }
     ...
     * the ACTIVITY interrupt (bit 8) of the IC_RAW_INTR_STAT register. Reset
     * value: 0x0
     */
    inline bool get_IC_CLR_ACTIVITY() volatile
    {
        return IC_CLR_ACTIVITY & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_STOP_DET() volatile
    {
        return IC_CLR_STOP_DET & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_START_DET() volatile
    {
        return IC_CLR_START_DET & 1u;
    }
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_GEN_CALL() volatile
    {
        return IC_CLR_GEN_CALL & 1u;
    }
     ...
     * DW_apb_i2c'. When DW_apb_i2c is disabled, the following occurs: - The TX
     * FIFO and RX FIFO get flushed. - Status bits in the IC_INTR_STAT register
     * are still active until DW_apb_i2c goes into IDLE state. If the module is
     * transmitting, it stops as well as deletes the contents of the transmit
     * buffer after the current transfer is complete. If the module is
     * receiving, the DW_apb_i2c stops the current transfer at the end of the
     * current byte and does not acknowledge the transfer. In systems with
     ...
     * refer to 'Disabling DW_apb_i2c' Reset value: 0x0
     */
     ...
     * cleared automatically after the abort operation. For a detailed
     * description on how to abort I2C transfers, refer to 'Aborting I2C
     * Transfers'. Reset value: 0x0
     */
     ...
     * MST_ACTIVITY bits. Reset value: 0x0
     */
     ...
     * time. None of the bits in this register request an interrupt. When the
     * I2C is disabled by writing 0 in bit 0 of the IC_ENABLE register: - Bits
     * 1 and 2 are set to 1 - Bits 3 and 10 are set to 0 When the master or
     * slave state machines goes to idle and ic_en=0: - Bits 5 and 6 are set to
     * 0
     */
     ...
     * transmit FIFO. Reset value: 0x0
     */
    inline uint8_t get_IC_TXFLR() volatile
    {
        return IC_TXFLR & 0b11111u;
    }
     ...
     * receive FIFO. Reset value: 0x0
     */
    inline uint8_t get_IC_RXFLR() volatile
    {
        return IC_RXFLR & 0b11111u;
    }
     ...
     * SCL goes from HIGH to LOW). The bits [23:16] of this register are used
     * to extend the SDA transition (if any) whenever SCL is HIGH in the
     * receiver in either master or slave mode. Writes to this register succeed
     ...
     * than the minimum hold time in each mode (one cycle in master mode, seven
     * cycles in slave mode) for the value to be implemented. The programmed
     * SDA hold time during transmit (IC_SDA_TX_HOLD) cannot exceed at any time
     * the duration of the low part of scl. Therefore the programmed value
     * cannot be larger than N_SCL_LOW-2, where N_SCL_LOW is the duration of
                                ...
                                uint8_t &IC_SDA_RX_HOLD) volatile
    {
        ...
        IC_SDA_TX_HOLD = curr & 0xffffu;
        IC_SDA_RX_HOLD = (curr >> 16u) & 0xffu;
    }
                                ...
                                uint8_t IC_SDA_RX_HOLD) volatile
    {
        ...
        curr &= ~(0xffffu);
        curr |= (IC_SDA_TX_HOLD & 0xffffu);
        curr &= ~(0xffu << 16u);
        curr |= (IC_SDA_RX_HOLD & 0xffu) << 16u;
     ...
     * first 10-bit address byte was not acknowledged by any slave. Reset
     * value: 0x0 Role of DW_apb_i2c:  Master-Transmitter or Master-Receiver
     */
     ...
     * any slave. Reset value: 0x0 Role of DW_apb_i2c:  Master-Transmitter or
     * Master-Receiver
     */
     ...
     * slave(s). Reset value: 0x0 Role of DW_apb_i2c:  Master-Transmitter
     */
     ...
     * Call and no slave on the bus acknowledged the General Call. Reset value:
     * 0x0 Role of DW_apb_i2c:  Master-Transmitter
     */
     ...
     * to be a read from the bus (IC_DATA_CMD[9] is set to 1). Reset value: 0x0
     ...
     * Speed Master code was acknowledged (wrong behavior). Reset value: 0x0
     ...
     * Byte was acknowledged (wrong behavior). Reset value: 0x0 Role of
     * DW_apb_i2c:  Master
     */
     ...
     * data in High Speed mode. Reset value: 0x0 Role of DW_apb_i2c:
     ...
     * START Byte. Reset value: 0x0 Role of DW_apb_i2c:  Master
     */
     ...
     * mode. Reset value: 0x0 Role of DW_apb_i2c:  Master-Receiver
     */
     ...
     * with the Master mode disabled. Reset value: 0x0 Role of DW_apb_i2c:
     * Master-Transmitter or Master-Receiver
     */
     ...
     * arbitration. Reset value: 0x0 Role of DW_apb_i2c:  Master-Transmitter or
     * Slave-Transmitter
     */
     ...
     * flush old data in TX FIFO. Reset value: 0x0 Role of DW_apb_i2c:
     * Slave-Transmitter
     */
     ...
     * longer own the bus. Reset value: 0x0 Role of DW_apb_i2c:
     * Slave-Transmitter
     */
     ...
     * IC_DATA_CMD register. Reset value: 0x0 Role of DW_apb_i2c:
     * Slave-Transmitter
     */
     ...
     * (IC_ENABLE[1]) Reset value: 0x0 Role of DW_apb_i2c:  Master-Transmitter
     */
     ...
     * disabled. Reset value: 0x0 Role of DW_apb_i2c:  Master-Transmitter or
     * Slave-Transmitter
     */
     ...
     * bit, Bit 9 clears for one cycle and is then re-asserted.
     */
     ...
     * is aborted and the data received is not pushed to the receive buffer.
     ...
     */
    inline bool get_IC_SLV_DATA_NACK_ONLY() volatile
    {
        return IC_SLV_DATA_NACK_ONLY & 1u;
    }
     ...
     * byte received - 0: generate NACK/ACK normally Reset value: 0x0
     */
    inline void set_IC_SLV_DATA_NACK_ONLY() volatile
    {
        IC_SLV_DATA_NACK_ONLY |= 1u;
    }
     ...
     * byte received - 0: generate NACK/ACK normally Reset value: 0x0
     */
    inline void clear_IC_SLV_DATA_NACK_ONLY() volatile
    {
        IC_SLV_DATA_NACK_ONLY &= ~(1u);
    }
     ...
     * byte received - 0: generate NACK/ACK normally Reset value: 0x0
     */
    inline void toggle_IC_SLV_DATA_NACK_ONLY() volatile
    {
        IC_SLV_DATA_NACK_ONLY ^= 1u;
    }
     ...
     * value, and TDMAE = 1. Reset value: 0x0
     */
    inline uint8_t get_IC_DMA_TDLR() volatile
    {
        return IC_DMA_TDLR & 0xfu;
    }
     ...
     * value, and TDMAE = 1. Reset value: 0x0
     */
    inline void set_IC_DMA_TDLR(uint8_t value) volatile
    {
     ...
     * when 1 or more data entries are present in the receive FIFO. Reset
     * value: 0x0
     */
    inline uint8_t get_IC_DMA_RDLR() volatile
    {
        return IC_DMA_RDLR & 0xfu;
    }
     ...
     * value: 0x0
     */
    inline void set_IC_DMA_RDLR(uint8_t value) volatile
    {
     ...
     * of 2.
     */
    inline uint8_t get_IC_SDA_SETUP() volatile
    {
        return IC_SDA_SETUP & 0xffu;
    }
     ...
     * of 2.
     */
    inline void set_IC_SDA_SETUP(uint8_t value) volatile
    {
     ...
     */
    inline bool get_IC_ACK_GENERAL_CALL() volatile
    {
        return IC_ACK_GENERAL_CALL & 1u;
    }
     ...
     * DW_apb_i2c responds with a NACK (by negating ic_data_oe).
     */
    inline void set_IC_ACK_GENERAL_CALL() volatile
    {
        IC_ACK_GENERAL_CALL |= 1u;
    }
     ...
     * DW_apb_i2c responds with a NACK (by negating ic_data_oe).
     */
    inline void clear_IC_ACK_GENERAL_CALL() volatile
    {
        IC_ACK_GENERAL_CALL &= ~(1u);
    }
     ...
     * DW_apb_i2c responds with a NACK (by negating ic_data_oe).
     */
    inline void toggle_IC_ACK_GENERAL_CALL() volatile
    {
        IC_ACK_GENERAL_CALL ^= 1u;
    }
     ...
     * SLV_DISABLED_WHILE_BUSY (bit 1). Reset value: 0x0
     */
    inline bool get_IC_ENABLE_STATUS_IC_EN() volatile
    {
        return IC_ENABLE_STATUS & 1u;
    }
     ...
     * CPU writes a 0 to the IC_ENABLE register while: (a) DW_apb_i2c is
     ...
     * operation from a remote master. When read as 1, DW_apb_i2c is deemed to
     * have forced a NACK during any part of an I2C transfer, irrespective of
     * whether the I2C address matches the slave address set in DW_apb_i2c
     * (IC_SAR register) OR if the transfer is completed before IC_ENABLE is
     * set to 0 but has not taken effect. Note:  If the remote I2C master
     * terminates the transfer with a STOP condition before the DW_apb_i2c has
     * a chance to NACK a transfer, and IC_ENABLE[0] has been set to 0, then
     * this bit will also be set to 1. When read as 0, DW_apb_i2c is deemed to
     * have been disabled when there is master activity, or when the I2C bus is
     * idle. Note:  The CPU can safely read this bit when IC_EN (bit 0) is read
     * as 0. Reset value: 0x0
     */
     ...
     * responded with a NACK. Note:  If the remote I2C master terminates the
     ...
     * also set to 1. When read as 0, DW_apb_i2c is deemed to have been
     * disabled without being actively involved in the data phase of a
     * Slave-Receiver transfer. Note:  The CPU can safely read this bit when
     * IC_EN (bit 0) is read as 0. Reset value: 0x0
     */
     ...
     * DW_apb_i2c is disabled. If IC_ENABLE[0] has been set to 1, bits 2:1 are
     * forced to 0, and bit 0 is forced to 1. If IC_ENABLE[0] has been set to
     ...
     */
    inline uint8_t get_IC_FS_SPKLEN() volatile
    {
        return IC_FS_SPKLEN & 0xffu;
    }
     ...
     * information, refer to 'Spike Suppression'.
     */
    inline void set_IC_FS_SPKLEN(uint8_t value) volatile
    {
     ...
     * IC_RAW_INTR_STAT register. Reset value: 0x0
     */
    inline bool get_IC_CLR_RESTART_DET() volatile
    {
        return IC_CLR_RESTART_DET & 1u;
    }
    ...
    {
        return (IC_COMP_PARAM_1 >> 8u) & 0xffu;
    }
    ...
    {
        return (IC_COMP_PARAM_1 >> 16u) & 0xffu;
    }
                                    ...
                                    uint8_t &TX_BUFFER_DEPTH) volatile
    {
        ...
        APB_DATA_WIDTH = curr & 0b11u;
        MAX_SPEED_MODE = (curr >> 2u) & 0b11u;
        HC_COUNT_VALUES = curr & (1u << 4u);
        INTR_IO = curr & (1u << 5u);
        HAS_DMA = curr & (1u << 6u);
        ADD_ENCODED_PARAMS = curr & (1u << 7u);
        RX_BUFFER_DEPTH = (curr >> 8u) & 0xffu;
        TX_BUFFER_DEPTH = (curr >> 16u) & 0xffu;
    }
};

static_assert(sizeof(i2c) == i2c::size);
static_assert(ifgen_struct<i2c>);

static volatile i2c *const I2C0_2040 = reinterpret_cast<i2c *>(0x40044000);

static volatile i2c *const I2C1_2040 = reinterpret_cast<i2c *>(0x40048000);

static volatile i2c *const I2C0_2350 = reinterpret_cast<i2c *>(0x40090000);

static volatile i2c *const I2C1_2350 = reinterpret_cast<i2c *>(0x40098000);

template <uint8_t instance>
inline volatile i2c *i2c_instance(void)
    requires(instance == 0)
{
    return I2C0_2040;
}
